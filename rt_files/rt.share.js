	...
	({
		get: {
			...
			ok: function(url, cb) {
				...
				window.ODKL.updateCount = function(idx, number) {
					$(".share-btn__ok").next("span").text(number);
				};
			...
			vk: function(url, cb) {
				...
				window.VK.Share = {
					count: function(idx, number) {
						$(".share-btn__vk").next("span").text(number);
					}
				};
			...
			gp: function(url, cb) {
                ...
                $.getScript('http://share.yandex.ru/gpp.xml?url=' + url, function() {
                    ...
                    if (result > 0) {
						$(".share-btn__gp").next("span").text(result);
					}
                });
			}
		},
		...
		init: function(p) {
			...
			this.get.tw(url, function(result){
				$(".share-btn__tw").next("span").text(result);
			});
			this.set.tw($(".share-btn__tw"), url);
			
			this.get.fb(url, function(result){
				$(".share-btn__fb-like").next("span").text(result[0].share_count);
			});
